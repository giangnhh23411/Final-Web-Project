...
class CartManager {
    ...
    // Hiển thị thông báo thông tin (sản phẩm đã có trong giỏ)
    showInfoMessage(message) {
        const notification = document.createElement('div');
        notification.className = 'cart-info-notification';
        notification.style.cssText = `
            position: fixed;
            top: 20px;
            right: 20px;
...
async function checkProductInCart(productId) {
    try {
        ...
        if (result.data && result.data.items) {
            // Tìm product_id trong items (có thể là ObjectId hoặc string)
        ...
        const localCart = JSON.parse(localStorage.getItem('phuclong_cart') || '[]');
        return localCart.some(item => item.id === productId);
    }
}

// Hàm thêm sản phẩm vào giỏ hàng (để sử dụng trong HTML) - GỌI API
async function addToCart(productId, productName, productPrice, productImage) {
    ...
    try {
        ...
        if (alreadyInCart) {
            cartManager.showInfoMessage(`${productName} đã có trong giỏ hàng của bạn`);
            return;
        }
        ...
        if (response.ok) {
            // Cập nhật LocalStorage cart để sync
            const product = {
                id: productId,
                name: productName,
            ...
            if (existingItem) {
                ...
                cartManager.cart.push(product);
            }
            cartManager.saveCart();
            
            // Hiển thị thông báo thành công
            cartManager.showSuccessMessage(productName);
        } else {
            // Lỗi từ API - fallback về localStorage
