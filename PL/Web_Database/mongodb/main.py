...
# Project / Figma paths (absolute project root as you provided)
PROJECT_ROOT = r"D:\Final-Web-Project\PL"
figma_dir = os.path.join(PROJECT_ROOT, "Figma")
AVATAR_DIR = os.path.join(figma_dir, "admin", "images", "avatar")
...
# Mount static Figma folder so frontend can request /Figma/...
if os.path.isdir(figma_dir):
    app.mount("/Figma", StaticFiles(directory=figma_dir), name="figma")
...
# Redirect root to UI entry
@app.get("/")
async def root_index():
    return RedirectResponse(url="/Figma/index.html")
...
class VoucherBase(BaseModel):
    ...
    type: Optional[str] = Field(None, enum=['percent', 'fixed'])
    ...
    status: Optional[str] = Field('active', enum=['active', 'expired', 'disabled'])
    applicable_product_ids: Optional[List[str]] = None

class VoucherCreate(VoucherBase):
    code: str = Field(..., description="Voucher code required")
    type: str = Field(..., enum=['percent', 'fixed'])
...
async def list_products(page: int = Query(1, ge=1), pageSize: int = Query(PAGE_SIZE_DEFAULT, ge=1, le=500)):
    skip = (page - 1) * pageSize
    cursor = products_col.find().skip(skip).limit(pageSize)
    items = []
    async for doc in cursor:
